...
namespace Service.WeiXinPayCallBack
{
    ...
    /// WeiXinPay_Xxw 的摘要说明
    /// </summary>
    public class WeiXinPay_Xxw : IHttpHandler, IRequiresSessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                if (IsValidate == false)
                {
                    Business.WeiXinPay_Xxw.Log("验签失败:" + CommunityId + "," + orderId.ToString());
                    Result = SetNotifyResult("FAIL", Result);
                    context.Response.Write(Result);
                    return;
                }

                Business.WeiXinPay_Xxw.Log("微信支付验签成功:" + CommunityId + "," + orderId.ToString());

                if (IsValidate == true)
                {
                    //初始化参数
                    new Business.WeiXinPay_Xxw().GenerateConfig(CommunityId);
                    //更新订单返回状态
                    Business.WeiXinPay_Xxw.UpdateProperyOrder(CommunityId, orderId, respcode, respmsg);
                    //已收到应答，无需再通知
                    context.Response.Write(SetNotifyResult("SUCCESS", "OK"));
                    if (respcode == "SUCCESS")
                    {
                        //下账
                        string ReceResult = Business.WeiXinPay_Xxw.ReceProperyOrder(CommunityId, orderId);
                        Business.WeiXinPay_Xxw.Log("微信支付下账:" + CommunityId.ToString() + "," + orderId + "," + ReceResult);
                    }

                    Business.WeiXinPay_Xxw.Log("微信支付流程:" + CommunityId.ToString() + "," + orderId + ",SUCCESS");
                }
            }
            catch (Exception E)
            {
                Business.WeiXinPay_Xxw.Log(E.Message.ToString());
                context.Response.ContentType = "text/plain";
