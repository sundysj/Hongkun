...
namespace Business
{
    public class Alipay : PubInfo
    {
        ...
        public Alipay() //获取小区、项目信息
        {
            base.Token = "20160804Alipay";
            c = new Config();
        ...
        public static string ReceProperyOrder(decimal total_amount, string CommunityId, string out_trade_no, string pointUseHistoryID)
        {
            ...
            {               
                record.RecordOperationLog(dAttributeTable.Rows[0], E.Message, "Alipay", "支付宝回调");
        ...
        public string GenerateOrder(DataRow Row)
        {
            ...
            using (var appConn = new SqlConnection(PubConstant.UnifiedContionString))
            {
                ...
                try
                {
                    log.Info("支付宝自助缴费下单，用户ID:" + CustID + ",应收费用项:" + FeesIds);
                    // 生成ERP账单
                    string PropertyResult = GeneratePropertyOrder(CommunityId, FeesIds, txnTime, CustID,RoomID, ref IsPropertyOk, ref Amount, ref PropertyOrderId);
                    ...
                    if (IsPropertyOk == true)
                    {
                        ...
                        // 支付宝签名订单信息
                        string BankResult = GenerateBankOrder(CommunityId, feesArray.First(), PropertyOrderId, txnTime, Amount, ref IsBankOk, ref prepay_str);
        ...
        /// <returns></returns>
        public string GenerateBankOrder(string CommunityId, string feesId, string out_trade_no, string txnTime, decimal total_fee, ref bool R, ref string prepay_str)
        {
        ...
        /// <returns></returns>
        public string GeneratePropertyOrder(string CommunityId, string FeesIds, string txnTime, string CustID,string RoomID, ref bool IsOk, ref decimal Amount, ref string PropertyOrderId)
        {
            ...
            if (CommunityTable.Rows.Count > 0)
            {
                ...
                {
                    Ds = PropertyOrder(CustID, FeesIds, T["CommID"].ToString(), c.seller_id.ToString(), txnTime, CommunityId,RoomID);
                }
        ...
        /// <returns></returns>
        public DataSet PropertyOrder(string CustId, string FeesIds, string CommID, string merId, string txnTime, string CommunityId,string RoomID)
        {
            string ConnStr = GetConnection(CommunityId);
            SqlParameter[] parameters = {
