...
namespace Business
{
    public class WeiXinPay : PubInfo
    {
        ...
        public WeiXinPay() //获取小区、项目信息
        {
            base.Token = "20160804WeiXinPay";
        }
        ...
        /// <param name="respmsg"></param>
        /// <returns></returns>
        public static string ReceProperyOrder(decimal total_amount, string CommunityId, string out_trade_no, string pointUseHistoryID,int? OrderType = null)
        {
            try
            {
                ...
                ReceFees(total_amount, CommunityId, out_trade_no, pointUseHistoryID,OrderType);
        ...
        /// <returns></returns>
        public static void ReceFees(decimal total_amount, string CommunityId, string OrderId, string pointUseHistoryID,int? OrderType =null)
        {
            List<SqlParameter> parameters = new List<SqlParameter>()
            {
                 ...
                 new SqlParameter("@OrderType",SqlDbType.VarChar){Value = OrderType.GetValueOrDefault().ToString()}
            };
            ...
            using (var appConn = new SqlConnection(PubConstant.UnifiedContionString))
            {
                ...
                try
                {
                    ...
                    if (IsPropertyOk == true)
                    {
                        ...
                        // 微信签名订单信息
                        WxPayData Data = new WxPayData();
                        string BankResult = GenerateBankOrder(CommunityId, feesArray.First(), PropertyOrderId, txnTime, Amount, ref IsBankOk, ref BankOrderId, ref Data, wxPayConfig, openId);
        ...
        /// <returns></returns>
        public string GenerateBankOrder(string CommunityId, string feesId, string out_trade_no, string txnTime, int total_fee, 
            ref bool R, ref string BankOrderId, ref WxPayData WPD, WxPayConfig wxPayConfig, string openId = "")
        {
            ...
            // 是否有使用积分
            data.SetValue("attach", CommunityId);
        ...
        /// <returns></returns>
        public string GeneratePropertyOrder(string CommunityId, string FeesIds, string txnTime, string CustID, 
            ref bool IsOk, ref int Amount, ref string PropertyOrderId, WxPayConfig wxPayConfig)
        {
            DataTable CommunityTable = new DbHelperSQLP(PubConstant.UnifiedContionString.ToString()).Query("SELECT * FROM Tb_Community WHERE Id='" + CommunityId.ToString() + "'").Tables[0];
