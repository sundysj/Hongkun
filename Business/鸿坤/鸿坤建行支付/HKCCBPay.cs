...
namespace Business
{
    public class HKCCBPay : PubInfo
    {
        ...
        /// <returns></returns>
        public static bool UpdateOrderInfoAlipay(string connStr, string OrderSN, int IsSucc, string Memo = null)
        {
            try
            {
                using (IDbConnection conn = new SqlConnection(connStr))
                {
                    return conn.Execute("UPDATE Tb_OL_AlipayOrder SET IsSucc=@IsSucc,trade_msg=@Memo WHERE out_trade_no = @out_trade_no", new { IsSucc = IsSucc, Memo= Memo, out_trade_no = OrderSN }) > 0;
                }
            }
            catch (Exception ex)
            {
        ...
        public static bool UpdateOrderDetailAlipay(string connStr, string OrderId)
        {
            try
            {
                using (IDbConnection conn = new SqlConnection(connStr))
                {
                    return conn.Execute("update Tb_OL_AlipayDetail set PaidAmount = DueAmount WHERE PayOrderId=@out_trade_no", new {out_trade_no = OrderId }) > 0;
                }
            }
            catch (Exception ex)
            {
