...
namespace Business
{
    ...
    /// 在线缴费
    /// </summary>
    public partial class CostInfo_HK : PubInfo
    {
        public CostInfo_HK()
        {
        ...
        private string PayFees(DataRow row)
        {
            ...
            if (!row.Table.Columns.Contains("ChargeMode") || string.IsNullOrEmpty(row["ChargeMode"].ToString()))
            {
                return JSONHelper.FromString(false, "缴费方式不能为空");
            }
            ...
            try
            {
                ...
                string memo = "";
                string ChargeMode = AppGlobal.StrToStr(row["ChargeMode"].ToString());
                long CustID = 0;
                long RoomID = 0;
        ...
        private string GetPrecCost(DataRow Row)
        {
            ...
            if ("f99195dc-9e4a-4094-9f1d-621b89ef6586".Equals(communityId))
            {
                foreach (DataRow item in dt.Select(" CostName <> '地面停车服务费'"))
                {
                    ...
                    dictionary.Add("value", item["CostID"]);
                    list.Add(dictionary);
                }
            }
            else
        ...
        private string GetArrearsList(DataRow row)
        {
            ...
            {
                sb.Append(" AND (x.SysCostSign='B0001' OR x.CostName = '公共照明' OR x.CostName = '生活垃圾清运') ");
            }
        ...
        private string GetBillList(DataRow row)
        {
            ...
            //构造查询语句
            StringBuilder sb = new StringBuilder();
            sb.Append(" select FeesID,CostName,DebtsAmount,DueAmount,StanMemo,SysCostSign,FeesChargeDate,FeesDueDate,ISNULL(isCharge,0) as isCharge, ISNULL(IsPrec,0) as IsPrec from view_HSPR_Fees_Filter ");
            sb.AppendFormat(" where CommID={0}", Community.CommID);
            sb.AppendFormat(" and CustID={0}", CustID);
            if (RoomID != "")
            {
