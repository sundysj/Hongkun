...
namespace Business
{
    public class UnionPayPrec : PubInfo
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(UnionPay));
        SDKConfig s { get; set; }
        public UnionPayPrec() //获取小区、项目信息
        {
            base.Token = "20160804UnionPayPrec";
            s = new SDKConfig();
            //银联后联地址
            ...
            s.BackUrl = "http://125.64.16.10:9999/TWInterface/Service/UnionPayCallBack/UnionPayPrec.ashx";
        }
        ...
        {
            DataTable CertificateTable = new DbHelperSQLP(PubConstant.UnifiedContionString.ToString()).Query("SELECT * FROM Tb_UnionPayCertificate WHERE CommunityId='" + CommunityId + "'").Tables[0];
        ...
        public string GeneratePropertyOrder(string CommunityId, string CostID, string RoomID,string txnTime, string CustID, ref bool IsOk, ref string Amount, ref string PropertyOrderId)
        {
            ...
            if (CommunityTable.Rows.Count > 0)
            {
                ...
                //生成物业订单
                DataSet Ds = PropertyOrder(CustID, CostID, T["CommID"].ToString(), RoomID,s.merId.ToString(), txnTime, CommunityId, Amount);
        ...
        public DataSet PropertyOrder(string CustId, string CostID, string CommID,string RoomID,string merId, string txnTime, string CommunityId,string PrecAmount)
        {
            ...
            DataSet Ds = new DbHelperSQLP(ConnStr).RunProcedure("Proc_OL_GenerateOrder_Prec", parameters, "Ds");
            return Ds;
        }
