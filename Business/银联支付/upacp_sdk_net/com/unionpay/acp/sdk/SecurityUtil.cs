...
namespace com.unionpay.acp.sdk
{
    public static class SecurityUtil
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static string EncryptPin(string pin, string card, Encoding encoding,SDKConfig con)
        {
            /** 生成PIN Block **/
            byte[] pinBlock = pin2PinBlockWithCardNO(pin, card);

            /** 使用公钥对密码加密 **/
            byte[] data = encryptedData(pinBlock,con);
        ...
        public static string EncryptData(string dataString, Encoding encoding, SDKConfig con)
        {
            /** 使用公钥对数据加密 **/
            byte[] data = encryptedData(encoding.GetBytes(dataString),con);
            return Convert.ToBase64String(data);
        }
