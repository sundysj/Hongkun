...
namespace com.unionpay.acp.sdk
{
    ...
    public class CertUtil
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static RSACryptoServiceProvider GetSignProviderFromPfx(SDKConfig con)
        {
             log.Debug("取配置文件证书");
             return GetSignProviderFromPfx(con.SignCertPath, con.SignCertPwd);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetSignCertId(SDKConfig con)
        {
            log.Debug("取配置文件证书");
            return GetSignCertId(con.SignCertPath, con.SignCertPwd);
        }

        private static void initEncryptCert(SDKConfig con)
        {
            log.Info("读取加密证书……");
            encryptCert = new Cert();
            encryptCert.cert = new X509Certificate2(con.EncryptCert);
        ...
        /// </summary>
        /// <returns></returns>
        public static RSACryptoServiceProvider GetEncryptKey(SDKConfig con)
        {
            if (encryptCert == null)
            {
                initEncryptCert( con);
        ...
        private static void initCerCerts(SDKConfig con)
        {
            log.Info("读取验签证书文件夹下所有cer文件……");
            DirectoryInfo directory = new DirectoryInfo(con.ValidateCertDir);
            FileInfo[] files = directory.GetFiles("*.cer");
            if (null == files || 0 == files.Length)
            {
                log.Info("请确定[" + con.ValidateCertDir + "]路径下是否存在cer文件");
                return;
            }
        ...
        /// <returns></returns>
        public static RSACryptoServiceProvider GetValidateProviderFromPath(string certId, SDKConfig con)// 
        {
