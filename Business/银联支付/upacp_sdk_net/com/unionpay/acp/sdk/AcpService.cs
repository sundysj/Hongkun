...
namespace com.unionpay.acp.sdk
{
    public class AcpService
    {
        ...
        /// <returns></returns>
        public static void Sign(Dictionary<string, string> reqData, Encoding encoding, SDKConfig s)
        {
            Sign(reqData, encoding, s.SignCertPath, s.SignCertPwd);
        }
        ...
        /// <returns></returns>
        public static bool Validate(Dictionary<string, string> rspData, Encoding encoding,SDKConfig con)
        {
            //获取签名
            ...
            RSACryptoServiceProvider provider = CertUtil.GetValidateProviderFromPath(rspData["certId"], con);
            if (null == provider)
            {
        ...
        /// <param name="encoding"></param>
        /// <returns></returns>
        public static bool ValidateAppResponse(string jsonData, Encoding encoding,SDKConfig con)
        {
            ...
            string stringSignDigest = BitConverter.ToString(signDigest).Replace("-", "").ToLower();
            log.Debug("sha1结果：[" + stringSignDigest + "]");
            RSACryptoServiceProvider provider = CertUtil.GetValidateProviderFromPath(dataMap["cert_id"],con);
            if (null == provider)
            {
                log.Error("未找到证书，无法验签，验签失败。");
                return false;
        ...
        public static Dictionary<String, String> Post(Dictionary<String, String> reqData, String reqUrl, Encoding encoding, SDKConfig s)
        {
            string postData = SDKUtil.CreateLinkString(reqData, false, true);
            byte[] byteArray = encoding.GetBytes(postData);
            try
            {
                if ("false".Equals(s.IfValidateRemoteCert)) //测试环境不验https证书
        ...
        /// <param name="encoding">编码</param>
        /// <returns>服务器返回的数据</returns>
        public static String Get(String reqUrl, Encoding encoding, SDKConfig s)
        {
        ...
        /// <param name="encoding">编码</param>
        /// <returns>string类型结果</returns>
        public static string GetCustomerInfoWithEncrypt(Dictionary<string, string> customerInfo, Encoding encoding, SDKConfig con)
        {
            if (customerInfo == null || customerInfo.Count == 0)
            {
                return "";
            ...
            if (encryptedInfo.Count != 0)
            {
                ...
                encryptedInfoStr = SecurityUtil.EncryptData(encryptedInfoStr, encoding,con);
        ...
        /// <param name="encoder"></param>
        /// <returns></returns>
        public static string EncryptData(string data, Encoding encoding , SDKConfig con)
        {
            return SecurityUtil.EncryptData(data, encoding,con);
        }
        ...
        public static Dictionary<string, string> ParseCustomerInfo(string customerInfoStr, Encoding encoding , SDKConfig s)
        {
            return ParseCustomerInfo(customerInfoStr, encoding, s.SignCertPath, s.SignCertPwd);
        }
