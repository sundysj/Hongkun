...
namespace Business
{
    public class UnionPay : PubInfo
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(UnionPay));

        SDKConfig s { get; set; }
        public UnionPay() //获取小区、项目信息
        {
            base.Token = "20160804UnionPay";
            s = new SDKConfig();
            //银联后联地址
        ...
        public static void ReceFees(string CommunityId,string OrderId)
        {
            ...
            new DbHelperSQLP(ConnStr).RunProcedure("Proc_OL_UnionPayReceFees", parameters);
        }
        ...
        {
            DataTable CertificateTable = new DbHelperSQLP(PubConstant.UnifiedContionString.ToString()).Query("SELECT * FROM Tb_UnionPayCertificate WHERE CommunityId='" + DataSecurity.FilteSQLStr(CommunityId)+"'").Tables[0];
        ...
        public string GeneratePropertyOrder(string CommunityId, string FeesIds, string txnTime, string CustID, ref bool IsOk,ref string Amount,ref string PropertyOrderId)
        {
            ...
            if(CommunityTable.Rows.Count>0)
            {
                ...
                //生成物业订单
                DataSet Ds = PropertyOrder(CustID, FeesIds, T["CommID"].ToString(), s.merId.ToString(), txnTime,CommunityId);
        ...
        public DataSet PropertyOrder(string CustId,string FeesIds, string CommID,string merId,string txnTime,string CommunityId)
        {
            ...
            DataSet Ds = new DbHelperSQLP(ConnStr).RunProcedure("Proc_OL_GenerateOrder", parameters, "Ds");
            return Ds;
        }
