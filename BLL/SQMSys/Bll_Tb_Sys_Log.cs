...
{
	/// <summary>
	/// 业务逻辑类Bll_Tb_Sys_Log 的摘要说明。
	/// </summary>
	public class Bll_Tb_Sys_Log
	{
		private readonly MobileSoft.DAL.SQMSys.Dal_Tb_Sys_Log dal=new MobileSoft.DAL.SQMSys.Dal_Tb_Sys_Log();
		public Bll_Tb_Sys_Log()
		{}
		#region  成员方法
		/// <summary>
		/// 是否存在该记录
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public MobileSoft.Model.SQMSys.Tb_Sys_Log GetModelByCache(long LogCode)
		{
			
			string CacheKey = "Tb_Sys_LogModel-" + LogCode;
			...
			return (MobileSoft.Model.SQMSys.Tb_Sys_Log)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MobileSoft.Model.SQMSys.Tb_Sys_Log> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MobileSoft.Model.SQMSys.Tb_Sys_Log> DataTableToList(DataTable dt)
		{
			List<MobileSoft.Model.SQMSys.Tb_Sys_Log> modelList = new List<MobileSoft.Model.SQMSys.Tb_Sys_Log>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MobileSoft.Model.SQMSys.Tb_Sys_Log model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new MobileSoft.Model.SQMSys.Tb_Sys_Log();
