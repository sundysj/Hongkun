...
{
	/// <summary>
	/// 业务逻辑类Bll_Tb_WorkFlow_FlowNodeTemp 的摘要说明。
	/// </summary>
	public class Bll_Tb_WorkFlow_FlowNodeTemp
	{
		private readonly MobileSoft.DAL.WorkFlow.Dal_Tb_WorkFlow_FlowNodeTemp dal=new MobileSoft.DAL.WorkFlow.Dal_Tb_WorkFlow_FlowNodeTemp();
		public Bll_Tb_WorkFlow_FlowNodeTemp()
		{}
		#region  成员方法
		/// <summary>
		/// 是否存在该记录
		...
		/// 更新一条数据
		/// </summary>
		public void Update(MobileSoft.Model.WorkFlow.Tb_WorkFlow_FlowNodeTemp model)
		{
			dal.Update(model);
		}
		...
		/// 得到一个对象实体，从缓存中。
		/// </summary>
		public MobileSoft.Model.WorkFlow.Tb_WorkFlow_FlowNodeTemp GetModelByCache(int InfoId)
		{
			
			string CacheKey = "Tb_WorkFlow_FlowNodeTempModel-" + InfoId;
			...
			return (MobileSoft.Model.WorkFlow.Tb_WorkFlow_FlowNodeTemp)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<MobileSoft.Model.WorkFlow.Tb_WorkFlow_FlowNodeTemp> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MobileSoft.Model.WorkFlow.Tb_WorkFlow_FlowNodeTemp> DataTableToList(DataTable dt)
		{
			List<MobileSoft.Model.WorkFlow.Tb_WorkFlow_FlowNodeTemp> modelList = new List<MobileSoft.Model.WorkFlow.Tb_WorkFlow_FlowNodeTemp>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MobileSoft.Model.WorkFlow.Tb_WorkFlow_FlowNodeTemp model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new MobileSoft.Model.WorkFlow.Tb_WorkFlow_FlowNodeTemp();
